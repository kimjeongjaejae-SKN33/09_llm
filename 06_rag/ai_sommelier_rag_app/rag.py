"""
AI Sommelier의 이미지 분석/검색/추천 RAG 파이프라인
...
def _load_project_environment() -> None:
    """
    현재 폴더로부터 상위 폴더로 이동하면서
    가장 가까운 .env를 일거와 환경 변수로 등록
    """
...
def describe_dish_flavor(payload: dict[str, Any]) -> Runnable:
    """이미지 URL을 받아서
        영어 한 문장의 풍미 query를 생성한 Runnable을 반환"""
...
def search_wines(dish_flavor: str) -> dict[str, str]:
    ...
    # - 지정된 index_name, namesapce가 같은 index로 연결
    ...
    # 유사도가 높은 리뷰(documents) 5개를 찾아서 반환
